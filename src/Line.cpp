...
Line::Line(glm::vec3        startPoint,
   ...
   , mEndPoint(endPoint)
   , mPosition(position)
   , mRotation(glm::angleAxis(glm::radians(angleOfRotInDeg), axisOfRot))
...
Line::Line(Line&& rhs) noexcept
   ...
   , mRotation(std::exchange(rhs.mRotation, glm::quat()))
...
void Line::setRotation(const glm::quat& rotation)
{
   mRotation = rotation;
   mCalculateModelMatrix = true;
}
...
void Line::rotateByMultiplyingCurrentRotationFromTheLeft(const glm::quat& rotation)
{
   mRotation = rotation * mRotation;
   mCalculateModelMatrix = true;
}

void Line::rotateByMultiplyingCurrentRotationFromTheRight(const glm::quat& rotation)
{
